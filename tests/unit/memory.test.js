...
describe('test memory data provider', () => {
  ...
  test('readFragmentData()', async () => {
    ...
    await writeFragmentData('234', '1', 'confirming fragment 1 (data)');

    const data = await readFragmentData('234', '1');
    expect(data).toEqual('confirming fragment 1 (data)');
  ...
  test('listFragments() writeFragment() writeFragmentData()', async () => {
    ...
    await writeFragmentData('ab', '1', 'confirming fragment 1');

    await writeFragment({ ownerId: 'ab', id: '2', fragment: 'fragment 2' });
    await writeFragmentData('ab', '2', 'confirming fragment 2');

    const idNum = await listFragments('ab');
    expect(Array.isArray(idNum)).toBe(true);
    expect(idNum).toEqual(['1', '2']);
